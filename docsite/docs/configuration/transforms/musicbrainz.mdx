...
Create an empty Musicbrainz API config in the `data` for your [stage configuration](/configuration/transforms/#configuring-stages) in your [AIO Config](/configuration?configType=aio#configuration-types):
...
<summary>Additional Musicbrainz Servers and Rate Limiting</summary>

Other, or _additional_, Musicbrainz Servers/Mirrors can be added to the API configuration. **If more than one server is defined then multi-scrobbler will load balance requests based on available rate capacity.**
...
Additional Musicbrainz servers can optionally be configured with rate limiting.

Rate limiting is defined by **max number of requests** within **timespan of N seconds.**

These constraints apply:

* Official Musicbrainz server (`https://musicbrainz.org` or when no `url` is defined) is hard-limited to `1 req/s`
* Brainzmash is hard-limited to `3 req/s`
...
* **Pros**
  * Higher rate limit than official Musicbrainz server (6req/s vs 1req/s)
  * Potentially less downtime due to distributed nature
* **Cons**
  * There is implicit trust in how BrainzMash handles your traffic:
    * you cannot decide which community server will process your traffic but your request is anonymized by the load balancer so your traffic cannot be tracked by any community server
  ...
  * Musicbrainz DB data is synchronized with a slight delay so new entries and recent edits will lag behind the official server by at most 3 hours
