...
```json5
{
    ...
    "art": true // will always apply art to Play (if stage provides art)
}
```
...
<DetailsAdmo type="tip" summary="Checking for Art">

To test if your Play/scrobble has *no* `art` associated with the data use an empty string for `art`:

```json5
{
  "when": [
    {
      "art": "", // run the stage when play has no art (album, track, or artist art)
    },
  ]
}
```

Otherwise, using a string/regex pattern for `art` will test if *any* art url includes that pattern.

EX: only run stage if art does *not* come from deezer, use deezer's cdn in a not regex statement:

```json5
{
  "when": [
    {
      "art": "/^((?!(dzcdn)).)*$/", // run the stage when play has no art (album, track, or artist art)
    },
  ]
}
```
...
* All parts (`artist` `album` `albumArtist` `duration` `meta` `title` `art`) of a `when` are `AND` conditions
