...
Multi-scrobbler **will not** throw an error if the environmental value is not found. It will instead leave the string as-is and log a warning (`WARN` level) with the names of the missing environmental variable names like so:

```
[2025-12-16 15:49:03.532 -0500] DEBUG  : [App] [Sources] [spotify Secrets] Matched: None | Unmatched: SPOTIFY_SECRET
```
