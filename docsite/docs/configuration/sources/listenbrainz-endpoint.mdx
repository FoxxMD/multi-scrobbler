...
<DetailsAdmo type="tip" summary="Standard Endpoints">

Multi-scrobbler emulates some [Listenbrainz API](https://listenbrainz.readthedocs.io/en/latest/users/api/index.html) endpoints to ensure client compatibility.

Most importantly, the scrobble endpoint is:

```
http://myMultiScrobblerIP:9078/1/submit-listens
```

All available endpoints are found in the API documentation section [**Listenbrainz Ingress**](/api/accept-a-listenbrainz-scrobble-standard/).
...
[**See API documentation for the slug endpoint for more information.**](/api/accept-a-listenbrainz-scrobble-slug/)
...
[Music Assistant](https://www.music-assistant.io) is an open source audio library manager that is commonly integrated into [Home Assistant](https://www.home-assistant.io/).
...
[TIDAL Hi-Fi](https://github.com/Mastermindzh/tidal-hifi) is a web version of [TIDAL](https://tidal.com/) packaged as a desktop application. The desktop implementation enables [additional features](https://github.com/Mastermindzh/tidal-hifi#features) not available in the web-only player, including activity tracking and scrobbling.

<details>

Hi-Fi has its own [documentation](https://github.com/Mastermindzh/tidal-hifi/blob/master/docs/custom-scrobbler.md) on how to setup Multi-Scrobbler. The instructions below are based off their docs.

Hi-Fi recommends using the [non-standard URL scenario](./?scenario=slug#setup).

Create a Listenbrainz Endpoint Source config that includes both a [Token](#token) and a [Slug](#multiple-sources).
...
<Tabs groupId="configType" queryString>
    ...
    services:
      multi-scrobbler:
        ...
        environment:
          # ...
          - LZE_TOKEN=tidal-hifi-scrobbles-2025
          - LZE_SLUG=tidal
    ```
    </TabItem>
    <TabItem value="file" label="File">
    ```json title="endpointlz.json"
    [
      {
...
Then, follow the steps to enable and configure Listenbrainz in TIDAL Hi-Fi Settings [outlined in the Hi-Fi docs.](https://github.com/Mastermindzh/tidal-hifi/blob/master/docs/custom-scrobbler.md#step-3-configure-tidal-hi-fi)

In those settings, ensure that:

* the ending of the **API URL** you set matches the `slug` you configured above EX `slug=tidal` => `http://localhost:9078/api/listenbrainz/tidal`
* the token you configured above matches the **User Token** you set in Hi-Fi
