...
<Tabs groupId="qsSource">
    ...
    <TabItem value="lastfm" label="Last.fm">
        ...
        Create a new [File](/configuration?configType=file#configuration-types) named `lastfm.json` in the `./config` directory that will be used with your [`docker-compose.yml` stack](#create-docker-compose-file) and copy the below contents, replacing placeholders with the data you got from Last.fm API account creation:
...
Open this section for instructions on how to do this. The instructions are customized depending on which [Source](#setup-sources) is currently selected.

<details>

<summary>Setup Musicbrainz</summary>
